...
document.getElementById('signupBtn').addEventListener('click', async () => {
  ...
  if (!fullName || !dob || !phone || !address || !email || !citizenship || !username || !password) {
    status.textContent = 'Fill in all required fields.';
    return;
  }
  if (!email.includes('@') || !email.includes('.')) {
    status.textContent = 'Enter a valid Gmail address.';
    return;
  }
  ...
  try {
    const res = await authAPI.signup({
      username, password,
      full_name: fullName, dob, phone, address, citizenship, email,
    });
